...
namespace block_stratumtwo_setup\form;

defined('MOODLE_INTERNAL') || die();
require_once("$CFG->libdir/formslib.php");
require_once(dirname(dirname(dirname(__FILE__))) .'/block_stratumtwo_setup.php');
...
class autosetup_form extends \moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('text', 'configurl', get_string('configurl', \block_stratumtwo_setup::STR_PLUGINNAME));
        $mform->setType('configurl', PARAM_NOTAGS);
        $mform->addHelpButton('configurl', 'configurl', \block_stratumtwo_setup::STR_PLUGINNAME);
        $mform->addRule('configurl', null, 'required', null, 'client');
        $mform->addRule('configurl', null, 'maxlength', 255, 'client');

        // server API key
        $mform->addElement('text', 'apikey', get_string('apikey', \block_stratumtwo_setup::STR_PLUGINNAME));
        $mform->setType('apikey', PARAM_NOTAGS);
        $mform->addHelpButton('apikey', 'apikey', \block_stratumtwo_setup::STR_PLUGINNAME);
        ...
        $mform->addElement('text', 'sectionnum',
                get_string('sectionnum', \block_stratumtwo_setup::STR_PLUGINNAME));
        $mform->setType('sectionnum', PARAM_INT);
        $mform->addHelpButton('sectionnum', 'sectionnum', \block_stratumtwo_setup::STR_PLUGINNAME);
        ...
        $this->add_action_buttons(true,
            get_string('apply', \block_stratumtwo_setup::STR_PLUGINNAME));
    }
}
