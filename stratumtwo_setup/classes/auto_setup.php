 ...
 */
class block_stratumtwo_setup_auto_setup {
    
    public function __construct() {
    }
    
    public static function send_get_request($url, $api_key = null) {
        $ch = curl_init();
    ...
    public static function configure_content_from_url($courseid, $sectionNumber, $url, $api_key = null) {
        $setup = new self();
        try {
            $response = self::send_get_request($url, $api_key);
        ...
        if ($conf === null) {
            return array(\get_string('configjsonparseerror', \block_stratumtwo_setup::STR_PLUGINNAME));
        }
    ...
    public function configure_content($courseid, $sectionNumber, $conf) {
        ...
        if (!isset($conf->modules) || !is_array($conf->modules)) {
            return array(\get_string('configmodulesmissing', \block_stratumtwo_setup::STR_PLUGINNAME));
        }
        ...
        if ($section_row === false || trim($section_row->sequence) == false) {
            $must_sort = false;
        } else {
            $must_sort = true;
        }
        ...
        if ($must_sort && empty($errors)) {
            ...
            \block_stratumtwo_setup_sort_activities_in_section($courseid, $sectionNumber);
    ...
    protected function configure_exercise_round($courseid, $sectionNumber, \stdClass $module,
        ...
        if (!isset($module->key)) {
            $errors[] = \get_string('configmodkeymissing', \block_stratumtwo_setup::STR_PLUGINNAME);
            return;
        }
    ...
    protected function configure_categories($courseid, stdClass $categoriesConf, &$errors) {
        ...
        foreach ($categoriesConf as $key => $cat) {
            if (!isset($cat->name)) {
                $errors[] = \get_string('configcatnamemissing', \block_stratumtwo_setup::STR_PLUGINNAME);
                continue;
            }
    ...
    protected function configure_exercises(array &$categories, \mod_stratumtwo_exercise_round $exround,
        ...
        foreach ($config as $o) {
            if (!isset($o->key)) {
                $errors[] = \get_string('configexercisekeymissing', \block_stratumtwo_setup::STR_PLUGINNAME);
                continue;
            }
            if (!isset($o->category)) {
                $errors[] = \get_string('configexercisecatmissing', \block_stratumtwo_setup::STR_PLUGINNAME);
                continue;
            }
            if (!isset($categories[$o->category])) {
                $errors[] = \get_string('configexerciseunknowncat', \block_stratumtwo_setup::STR_PLUGINNAME, $o->category);
                continue;
            }
            if (!isset($o->max_submissions)) {
                $errors[] = \get_string('configchapternotsupported', \block_stratumtwo_setup::STR_PLUGINNAME);
                continue;
            }
    ...
    protected function parseExerciseStatus($value, &$errors) {
        switch ($value) {
            ...
            case 'maintenance':
                ...
                break;
            default:
                $errors[] = \get_string('configbadstatus', \block_stratumtwo_setup::STR_PLUGINNAME, $value);
                return \mod_stratumtwo_exercise::STATUS_HIDDEN;
        }
    }
    ...
    protected function parseInt($value, &$errors) {
        if (\is_numeric($value))
            ...
            $errors[] = \get_string('configbadint', \block_stratumtwo_setup::STR_PLUGINNAME, $value);
            return null;
        }
    }
    ...
    protected function parseFloat($value, &$errors) {
        if (\is_numeric($value))
            ...
            $errors[] = \get_string('configbadfloat', \block_stratumtwo_setup::STR_PLUGINNAME, $value);
            return null;
        }
    }
    ...
    protected function parseDate($value, &$errors) {
        ...
        $errors[] = \get_string('configbaddate', \block_stratumtwo_setup::STR_PLUGINNAME, $value);
        return null;
    }
    ...
    protected function parseDuration($openingTime, $duration, &$errors) {
        ...
        $errors[] = \get_string('configbadduration', \block_stratumtwo_setup::STR_PLUGINNAME, $duration);
        return null;
    }
}